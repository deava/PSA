...
exports.assignSuperadminRole = assignSuperadminRole;
exports.checkSuperadminRole = checkSuperadminRole;
exports.removeSuperadminRole = removeSuperadminRole;
exports.assignSuperadminRoleByEmail = assignSuperadminRoleByEmail;
exports.checkSuperadminRoleByEmail = checkSuperadminRoleByEmail;
exports.removeSuperadminRoleByEmail = removeSuperadminRoleByEmail;
...
async function assignSuperadminRole() {
    try {
        ...
        debug_logger_1.logger.info('Assigning superadmin role to user', { userId: user.id });
        // First, ensure the System department exists
        debug_logger_1.logger.debug('Step 1: Creating/checking System Administration department', {});
        ...
        if (checkDeptError && checkDeptError.code !== 'PGRST116') {
            ...
            debug_logger_1.logger.error('Error checking for existing department', {}, checkDeptError);
            throw new Error(`Failed to check for existing department: ${checkDeptError.message}`);
        }
        if (!existingDept) {
            // Department doesn't exist, create it
            debug_logger_1.logger.debug('System Administration department does not exist, creating', {});
            ...
            debug_logger_1.logger.debug('System Administration department already exists, skipping creation', {});
        }
        if (deptError) {
            debug_logger_1.logger.error('Error creating system department', {
        ...
        debug_logger_1.logger.debug('Step 2: Getting System Administration department ID', {});
        ...
        if (!deptData) {
            ...
            const { data: queryDeptData, error: deptQueryError } = await supabase
                ...
                .single();
            if (deptQueryError) {
                debug_logger_1.logger.error('Error querying system department', {
        ...
        debug_logger_1.logger.debug('Step 3: Creating Superadmin role', {});
        ...
        if (checkError && checkError.code !== 'PGRST116') {
            // PGRST116 means no rows found, which is fine
            debug_logger_1.logger.error('Error checking for existing superadmin role', {}, checkError);
            throw new Error(`Failed to check for existing superadmin role: ${checkError.message}`);
        }
        let roleError = null;
        if (!existingRole) {
            // Role doesn't exist, create it
            debug_logger_1.logger.debug('Superadmin role does not exist, creating', {});
            const { error } = await supabase
                .from('roles')
                .insert({
                name: 'Superadmin',
            ...
            debug_logger_1.logger.debug('Superadmin role already exists, skipping creation', {});
        }
        if (roleError) {
            debug_logger_1.logger.error('Error creating superadmin role', {
        ...
        debug_logger_1.logger.debug('Step 4: Getting Superadmin role ID', {});
        ...
        if (!roleData) {
            ...
            const { data: queryRoleData, error: roleQueryError } = await supabase
                ...
                .single();
            if (roleQueryError) {
                debug_logger_1.logger.error('Error querying superadmin role', {
        ...
        debug_logger_1.logger.debug('Step 5: Assigning Superadmin role to user', { userId: user.id, roleId: roleData.id });
        ...
        if (assignError) {
            debug_logger_1.logger.error('Error assigning superadmin role', {
            ...
            if (assignError.code === '23505') { // Unique constraint violation
                debug_logger_1.logger.info('Superadmin role already assigned to user', {});
        ...
        debug_logger_1.logger.info('Superadmin role assigned successfully', {});
        return { success: true, message: 'Superadmin role assigned successfully' };
    }
    catch (error) {
        debug_logger_1.logger.error('Error assigning superadmin role', {}, error);
        throw error;
    }
}
/**
...
async function checkSuperadminRole() {
    try {
      ...
      `)
        ...
        if (error) {
            debug_logger_1.logger.error('Error checking superadmin role', {}, error);
            return false;
        }
 ...
 */
async function removeSuperadminRole() {
    try {
        const supabase = (0, supabase_1.createClientSupabase)();
        if (!supabase) {
            throw new Error('Supabase not configured');
        ...
        debug_logger_1.logger.info('Superadmin role removed successfully', {});
        return { success: true, message: 'Superadmin role removed successfully' };
    }
    catch (error) {
        debug_logger_1.logger.error('Error removing superadmin role', {}, error);
        throw error;
    }
}
/**
 ...
 */
async function assignSuperadminRoleByEmail(email) {
    try {
        const supabase = (0, supabase_1.createClientSupabase)();
        if (!supabase) {
            throw new Error('Supabase not configured');
        }
        debug_logger_1.logger.info('Assigning superadmin role to email', { email });
        ...
        debug_logger_1.logger.debug('Found user', { userId: userData.id });
        ...
        if (assignError) {
            debug_logger_1.logger.error('Error assigning superadmin role', {}, assignError);
            return { success: false, message: `Failed to assign superadmin role: ${assignError.message}` };
        }
        debug_logger_1.logger.info('Superadmin role assigned successfully', { email });
 ...
 */
async function checkSuperadminRoleByEmail(email) {
    try {
        const supabase = (0, supabase_1.createClientSupabase)();
        if (!supabase) {
            throw new Error('Supabase not configured');
 ...
 */
async function removeSuperadminRoleByEmail(email) {
    try {
        const supabase = (0, supabase_1.createClientSupabase)();
        if (!supabase) {
            throw new Error('Supabase not configured');
        ...
        const { error: removeError } = await supabase
            ...
            .eq('role_id', roleData.id);
        if (removeError) {
            debug_logger_1.logger.error('Error removing superadmin role', {}, removeError);
            return { success: false, message: `Failed to remove superadmin role: ${removeError.message}` };
        }
        debug_logger_1.logger.info('Superadmin role removed successfully', { email });
