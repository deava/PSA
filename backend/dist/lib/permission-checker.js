...
exports.isAssignedToProject = isAssignedToProject;
exports.managesDepartment = managesDepartment;
exports.hasAccountAccess = hasAccountAccess;
exports.isAssignedToWorkflowNode = isAssignedToWorkflowNode;
exports.checkPermissionHybrid = checkPermissionHybrid;
 ...
 */
function getCacheKey(userId, permission, context) {
 ...
 */
async function isAssignedToProject(userId, projectId, supabaseClient) {
    ...
    if (projectFetchError && projectFetchError.code !== 'PGRST116') {
        debug_logger_1.logger.error('Error fetching project for assignment check', { userId, projectId }, projectFetchError);
    }
    ...
    if (projectError && projectError.code !== 'PGRST116') { // PGRST116 = no rows
        debug_logger_1.logger.error('Error checking project assignment', { userId, projectId }, projectError);
    }
    ...
    if (taskError && taskError.code !== 'PGRST116') {
        debug_logger_1.logger.error('Error checking task assignment for project access', { userId, projectId }, taskError);
    }
 ...
 */
async function managesDepartment(userProfile, departmentId, _supabaseClient) {
 ...
 */
async function getAccountProjects(accountId, supabaseClient) {
    const supabase = supabaseClient || (0, supabase_1.createClientSupabase)();
    if (!supabase)
        return [];
    ...
    if (error) {
        debug_logger_1.logger.error('Error fetching account projects', { accountId }, error);
        return [];
    }
 ...
 * @param supabaseClient - Optional authenticated Supabase client (REQUIRED for server-side, optional for client-side)
 */
async function hasAccountAccess(userId, accountId, supabaseClient) {
    const supabase = supabaseClient || (0, supabase_1.createClientSupabase)();
    if (!supabase)
        return false;
    ...
    const { data: taskAssignments, error: taskError } = await supabase
        ...
        .limit(1);
    if (taskError) {
        debug_logger_1.logger.error('Error checking task assignments for account access', { userId, accountId }, taskError);
    }
 ...
 * @param supabaseClient - Optional authenticated Supabase client
 */
async function isAssignedToWorkflowNode(userProfile, workflowInstanceId, supabaseClient) {
    if (!userProfile)
        return false;
    const supabase = supabaseClient || (0, supabase_1.createClientSupabase)();
    if (!supabase)
    ...
    if (instanceError || !instance || !instance.workflow_nodes) {
        debug_logger_1.logger.error('Error fetching workflow instance for node assignment check', { workflowInstanceId }, instanceError);
        return false;
    }
    ...
    if (currentNode.node_type === 'role' && currentNode.entity_id) {
        ...
        if (!userProfile.user_roles)
            return false;
        return userProfile.user_roles.some((ur) => ur.role_id === currentNode.entity_id);
    }
 ...
 * @param supabaseClient - Optional authenticated Supabase client
 */
async function hasBasePermission(userProfile, permission, supabaseClient) {
    if (!userProfile?.user_roles || userProfile.user_roles.length === 0) {
        debug_logger_1.logger.debug('No user roles found', { userId: userProfile?.id, permission });
        return false;
    }
    ...
    for (const userRole of userProfile.user_roles) {
        ...
        if (role.permissions) {
            ...
            if (process.env.NODE_ENV === 'development' && (permission === 'manage_time' || permission === 'view_newsletters')) {
                debug_logger_1.logger.debug('Permission check DETAIL', {
                    permission,
                    roleName: role.name,
                    roleId: role.id,
            ...
            if (permissions[permission] === true) {
                debug_logger_1.logger.debug('Permission found in loaded role (OR logic)', {
    ...
    if (roleIds.length === 0) {
        debug_logger_1.logger.debug('No valid role IDs found', { userId: userProfile.id, permission });
        return false;
    }
    const supabase = supabaseClient || (0, supabase_1.createClientSupabase)();
    if (!supabase) {
        debug_logger_1.logger.error('Supabase client not available', { userId: userProfile.id, permission });
        return false;
    }
    const { data: roles, error } = await supabase
        ...
        .in('id', roleIds);
    if (error) {
        debug_logger_1.logger.error('Error fetching role permissions', { userId: userProfile.id }, error);
        return false;
    }
    ...
    const hasPermission = roles?.some((role) => {
        ...
        if (permValue === true) {
            debug_logger_1.logger.debug('Permission found in database role (OR logic)', {
                userId: userProfile.id,
                permission,
                roleName: role.name,
                roleId: role.id
    ...
    debug_logger_1.logger.debug('Permission check result from database', {
        userId: userProfile.id,
        permission,
        hasPermission,
 ...
 */
async function checkPermissionHybrid(userProfile, permission, context, supabaseClient) {
    const startTime = Date.now();
    try {
        // 1. Null check
        if (!userProfile) {
            debug_logger_1.logger.debug('No user profile provided', { permission });
            return false;
        }
        // 2. Superadmin bypass
        if (isSuperadmin(userProfile)) {
            (0, debug_logger_1.permissionCheck)(permission, userProfile.id, true, { reason: 'superadmin', duration: Date.now() - startTime });
            return true;
        }
        // 3. Check cache
        clearExpiredCache();
        ...
        if (cached) {
            (0, debug_logger_1.permissionCheck)(permission, userProfile.id, cached.result, { cached: true, duration: Date.now() - startTime });
            return cached.result;
        }
        // 4. Define override permissions map (used for both base check fallback and context bypass)
        const overridePermissions = {
            // Projects
            [permissions_1.Permission.VIEW_PROJECTS]: [permissions_1.Permission.VIEW_ALL_PROJECTS],
            [permissions_1.Permission.MANAGE_PROJECTS]: [permissions_1.Permission.MANAGE_ALL_PROJECTS],
            // Departments
            [permissions_1.Permission.VIEW_DEPARTMENTS]: [permissions_1.Permission.VIEW_ALL_DEPARTMENTS],
            ...
            // Accounts
            [permissions_1.Permission.VIEW_ACCOUNTS]: [permissions_1.Permission.VIEW_ALL_ACCOUNTS],
            // MANAGE_ACCOUNTS has no override — requires base permission + context
            // (VIEW_ALL_ACCOUNTS should NOT grant manage privileges)
            // Updates
            [permissions_1.Permission.VIEW_UPDATES]: [permissions_1.Permission.VIEW_ALL_UPDATES],
            // MANAGE_UPDATES has no override — requires base permission + context
            // Time Tracking
            [permissions_1.Permission.VIEW_TIME_ENTRIES]: [permissions_1.Permission.VIEW_ALL_TIME_ENTRIES],
            [permissions_1.Permission.EDIT_TIME_ENTRIES]: [permissions_1.Permission.VIEW_ALL_TIME_ENTRIES],
            // Note: MANAGE_TIME intentionally has no override — VIEW_ALL_TIME_ENTRIES should NOT grant write access
            // Workflows
            [permissions_1.Permission.MANAGE_WORKFLOWS]: [permissions_1.Permission.MANAGE_ALL_WORKFLOWS],
            [permissions_1.Permission.EXECUTE_WORKFLOWS]: [permissions_1.Permission.EXECUTE_ANY_WORKFLOW, permissions_1.Permission.MANAGE_ALL_WORKFLOWS],
            // Analytics (three-tier hierarchy: dept → account → org)
            [permissions_1.Permission.VIEW_ALL_DEPARTMENT_ANALYTICS]: [permissions_1.Permission.VIEW_ALL_ACCOUNT_ANALYTICS, permissions_1.Permission.VIEW_ALL_ANALYTICS],
            [permissions_1.Permission.VIEW_ALL_ACCOUNT_ANALYTICS]: [permissions_1.Permission.VIEW_ALL_ANALYTICS],
            // Capacity
            [permissions_1.Permission.VIEW_TEAM_CAPACITY]: [permissions_1.Permission.VIEW_ALL_CAPACITY],
        };
        ...
        if (!hasBase && !hasOverride) {
            ...
            (0, debug_logger_1.permissionCheck)(permission, userProfile.id, false, { reason: 'no_base_permission', duration });
            return false;
        }
        ...
        if (hasOverride) {
            ...
            (0, debug_logger_1.permissionCheck)(permission, userProfile.id, true, { reason: 'override_permission', duration });
            return true;
        }
        ...
        if (!context || Object.keys(context).length === 0) {
            const duration = Date.now() - startTime;
            permissionCache.set(cacheKey, { result: true, timestamp: Date.now() });
            (0, debug_logger_1.permissionCheck)(permission, userProfile.id, true, { reason: 'base_permission', duration });
            return true;
        }
        ...
        (0, debug_logger_1.permissionCheck)(permission, userProfile.id, hasAccess, {
        ...
        const duration = Date.now() - startTime;
        debug_logger_1.logger.error('Exception in checkPermissionHybrid', {
            permission,
            userId: userProfile?.id,
            context,
            duration
 ...
 */
async function checkAnyPermission(userProfile, permissions, context, supabaseClient) {
 ...
 */
async function checkAllPermissions(userProfile, permissions, context, supabaseClient) {
    for (const permission of permissions) {
        if (!(await checkPermissionHybrid(userProfile, permission, context, supabaseClient))) {
            return false;
        }
 ...
 * @param supabaseClient - Optional authenticated Supabase client
 */
async function getUserPermissions(userProfile, supabaseClient) {
    if (!userProfile)
        return [];
    ...
    roles.forEach((role) => {
        ...
        Object.entries(permissions).forEach(([perm, enabled]) => {
            if (enabled && Object.values(permissions_1.Permission).includes(perm)) {
...
exports.hasPermission = checkPermissionHybrid;
