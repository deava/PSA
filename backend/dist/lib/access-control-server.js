...
exports.isAssignedToProjectServer = isAssignedToProjectServer;
exports.getProjectIdFromWorkflowInstance = getProjectIdFromWorkflowInstance;
exports.getProjectIdFromWorkflowHistory = getProjectIdFromWorkflowHistory;
exports.hasAccountAccessServer = hasAccountAccessServer;
exports.verifyWorkflowInstanceAccess = verifyWorkflowInstanceAccess;
exports.verifyWorkflowHistoryAccess = verifyWorkflowHistoryAccess;
exports.verifyFormResponseAccess = verifyFormResponseAccess;
const debug_logger_1 = require("./debug-logger");
/**
 * Check if user is assigned to a specific project
 * Server-side version that accepts supabase client as parameter
 */
async function isAssignedToProjectServer(supabase, userId, projectId) {
 ...
 */
async function getProjectIdFromWorkflowInstance(supabase, workflowInstanceId) {
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data) {
        debug_logger_1.logger.error('Error fetching workflow instance for access check', { workflowInstanceId }, error);
        return null;
    }
 ...
 */
async function getProjectIdFromWorkflowHistory(supabase, historyId) {
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data) {
        debug_logger_1.logger.error('Error fetching workflow history for access check', { historyId }, error);
        return null;
    }
    // workflow_instances could be a single object or an array
    const workflowInstance = data.workflow_instances;
    if (!workflowInstance)
        return null;
 ...
 */
async function hasAccountAccessServer(supabase, userId, accountId) {
 ...
 */
async function verifyWorkflowInstanceAccess(supabase, userId, workflowInstanceId) {
 ...
 */
async function verifyWorkflowHistoryAccess(supabase, userId, historyId) {
 ...
 */
async function verifyFormResponseAccess(supabase, userId, formResponseId) {
