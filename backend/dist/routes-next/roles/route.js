...
async function GET(request) {
    try {
        ...
        if (!supabase) {
            debug_logger_1.logger.error('Failed to create Supabase client', { action: 'getRoles' });
            return server_1.NextResponse.json({ error: 'Failed to create database connection' }, { status: 500 });
        }
      ...
      `)
        ...
        if (error) {
            debug_logger_1.logger.error('Error fetching roles', {
                action: 'getRoles',
                code: error.code,
            }, error);
...
async function processRolesData(supabase, roles) {
    ...
    if (!roles || roles.length === 0) {
        debug_logger_1.logger.warn('No roles found - may be RLS policy blocking access or empty database', { action: 'getRoles' });
        return server_1.NextResponse.json({
            roles: [],
            containers: [],
            totalRoles: 0,
    ...
    const rolesWithData = await Promise.all(roles.map(async (role) => {
        ...
        `)
            .eq('role_id', role.id);
        const users = userError ? [] : userRoles?.map((ur) => ur.user_profiles).filter(Boolean) || [];
...
async function POST(request) {
    ...
    try {
        ...
        const supabase = (0, supabase_server_1.createApiSupabaseClient)(request);
        if (!supabase) {
            debug_logger_1.logger.error('Supabase not configured', { action: 'createRole' });
        ...
        (0, debug_logger_1.apiCall)('POST', '/api/roles', { action: 'createRole', name, department_id });
        // Validate input data
        const validation = (0, validation_1.validateRole)({
            name,
            description,
            department_id,
            permissions,
        ...
        (0, debug_logger_1.databaseQuery)('INSERT', 'roles', { action: 'createRole', name });
        ...
        if (isSystemRole) {
            ...
            const { data: reportingRole, error: reportingError } = await supabase
                ...
                .single();
            if (reportingError) {
                debug_logger_1.logger.error('Error fetching reporting role', {
                    action: 'createRole',
                    reporting_role_id,
        ...
        debug_logger_1.logger.info('Calculated hierarchy level', {
            action: 'createRole',
            name,
            hierarchy_level,
            display_order,
        ...
        const { data: role, error } = await supabase
            ...
            .single();
        if (error) {
            (0, debug_logger_1.databaseError)('INSERT', 'roles', error, { action: 'createRole', name });
        ...
        if (role.hierarchy_level !== hierarchy_level) {
            if (process.env.NODE_ENV === 'development') {
                debug_logger_1.logger.debug('DATABASE TRIGGER OVERRIDE: Expected level mismatch', { data: { expected: hierarchy_level, got: role.hierarchy_level } });
            }
            ...
            if (correctLevel !== role.hierarchy_level) {
                if (process.env.NODE_ENV === 'development') {
                    debug_logger_1.logger.debug('Correcting created role hierarchy level', { data: { from: role.hierarchy_level, to: correctLevel } });
                }
                ...
                const { error: correctionError } = await supabase
                    ...
                    .eq('id', role.id);
                if (correctionError) {
                    debug_logger_1.logger.error('Error correcting created role hierarchy level', {}, correctionError);
                }
                else {
                    debug_logger_1.logger.debug('Created role hierarchy level corrected successfully');
        ...
        (0, debug_logger_1.apiResponse)('POST', '/api/roles', 200, {
            action: 'createRole',
            duration,
            roleId: role.id
        });
        ...
        return server_1.NextResponse.json({ role });
    }
    catch (error) {
        return (0, server_guards_1.handleGuardError)(error);
    }
