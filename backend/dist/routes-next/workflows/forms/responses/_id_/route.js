...
async function GET(request, { params }) {
    ...
    try {
      ...
      `)
        ...
        const accessCheck = await (0, access_control_server_1.verifyFormResponseAccess)(supabase, user.id, id);
        ...
        // Get form response
        const response = await (0, form_service_1.getFormResponseById)(id);
        if (!response) {
            return server_1.NextResponse.json({ error: 'Form response not found' }, { status: 404 });
        }
        return server_1.NextResponse.json({ success: true, response }, { status: 200 });
    }
    catch (error) {
        debug_logger_1.logger.error('Error in GET /api/workflows/forms/responses/[id]', {}, error);
