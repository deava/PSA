...
async function GET(request, { params }) {
    ...
    try {
      ...
      `)
        ...
        const instance = await (0, workflow_service_1.getWorkflowInstanceById)(id);
        if (!instance) {
            return server_1.NextResponse.json({ error: 'Workflow instance not found' }, { status: 404 });
        }
        return server_1.NextResponse.json({ success: true, instance }, { status: 200 });
    }
    catch (error) {
        debug_logger_1.logger.error('Error in GET /api/workflows/instances/[id]', {}, error);
