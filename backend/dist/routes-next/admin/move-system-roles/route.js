...
async function POST(request) {
    try {
        ...
        const userProfile = await (0, auth_server_1.getCurrentUserProfileServer)();
        if (!userProfile || !(0, rbac_1.isSuperadmin)(userProfile)) {
            return server_1.NextResponse.json({ error: 'Unauthorized - Superadmin access required' }, { status: 403 });
        }
        ...
        if (systemDeptError || !systemDept) {
            return server_1.NextResponse.json({ error: 'System department not found' }, { status: 404 });
        }
        ...
        if (internalAffairsError || !internalAffairsDept) {
            ...
            if (createError || !newDept) {
                return server_1.NextResponse.json({ error: 'Failed to create Internal Affairs department' }, { status: 500 });
            }
            finalInternalAffairsDept = newDept;
        }
        // Ensure we have a valid department
        if (!finalInternalAffairsDept) {
            return server_1.NextResponse.json({ error: 'Internal Affairs department not found or created' }, { status: 500 });
        }
        ...
        if (rolesError) {
            return server_1.NextResponse.json({ error: 'Failed to fetch system roles' }, { status: 500 });
        }
        ...
        const { error: updateError } = await supabase
            ...
            .in('id', roleIds);
        if (updateError) {
            return server_1.NextResponse.json({ error: 'Failed to update roles' }, { status: 500 });
        }
        return server_1.NextResponse.json({
        ...
        debug_logger_1.logger.error('Error in POST /api/admin/move-system-roles:', {}, error);
