    ...
    try {
      ...
      `)
        ...
        try {
            ...
            debug_logger_1.logger.error('Error parsing request body', {}, parseError);
            return server_1.NextResponse.json({ error: 'Invalid JSON in request body' }, { status: 400 });
        }
        const { nodes, edges } = body;
        // Debug logging
        debug_logger_1.logger.debug('[Workflow Save] Starting save for template', { templateId });
        debug_logger_1.logger.debug('[Workflow Save] Received nodes', { count: nodes?.length || 0 });
        debug_logger_1.logger.debug('[Workflow Save] Received edges', { count: edges?.length || 0 });
        if (!nodes || !Array.isArray(nodes)) {
            return server_1.NextResponse.json({ error: 'Invalid nodes data - must be an array' }, { status: 400 });
        }
        // Validate node IDs are valid UUIDs
        const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        for (const node of nodes) {
            if (!node.id || !uuidRegex.test(node.id)) {
                debug_logger_1.logger.error('[Workflow Save] Invalid node ID format', { nodeId: node.id });
        ...
        if (edges && Array.isArray(edges)) {
            ...
            for (const node of nodes) {
                ...
                if (nodeType === 'end')
                    continue; // End nodes have no outgoing edges
                if (BRANCHING_NODE_TYPES.includes(nodeType))
                    continue; // These can branch
        ...
        if (entityIds.length > 0) {
            ...
            if (roleIds.length > 0) {
                ...
                if (missingRoles.length > 0) {
                    const affectedNodes = nodes
                        .filter((n) => {
                        const config = n.data?.config;
                        return missingRoles.includes((config?.roleId || config?.approverRoleId));
                    })
            ...
            const departmentIds = nodes
                .filter((n) => n.data?.config?.departmentId)
                .map((n) => n.data?.config?.departmentId)
                .filter(Boolean);
            if (departmentIds.length > 0) {
                ...
                if (missingDepts.length > 0) {
                    const affectedNodes = nodes
                        .filter((n) => {
                        const config = n.data?.config;
                        return missingDepts.includes(config?.departmentId);
                    })
                        .map((n) => n.data?.label || n.id);
                    return server_1.NextResponse.json({
        ...
        debug_logger_1.logger.debug('[Workflow Save] Verifying template exists', { templateId });
        ...
        if (templateError || !template) {
            debug_logger_1.logger.error('[Workflow Save] Template not found', {}, templateError);
            return server_1.NextResponse.json({ error: 'Workflow template not found' }, { status: 404 });
        }
        debug_logger_1.logger.debug('[Workflow Save] Template verified');
        // First, get the node IDs that belong to this template
        debug_logger_1.logger.debug('[Workflow Save] Getting existing node IDs');
        const { data: existingNodes } = await admin
            .from('workflow_nodes')
            .select('id')
            .eq('workflow_template_id', templateId);
        const existingNodeIds = existingNodes?.map(n => n.id) || [];
        debug_logger_1.logger.debug('[Workflow Save] Found existing nodes', { count: existingNodeIds.length });
        // Clear ALL FK references to these nodes before deleting them
        if (existingNodeIds.length > 0) {
            debug_logger_1.logger.debug('[Workflow Save] Clearing FK references for node deletion', { count: existingNodeIds.length });
        ...
        debug_logger_1.logger.debug('[Workflow Save] Deleting existing nodes');
        const { error: deleteError } = await admin
            .from('workflow_nodes')
            .delete()
            .eq('workflow_template_id', templateId);
        if (deleteError) {
            debug_logger_1.logger.error('[Workflow Save] Error deleting existing nodes', {}, deleteError);
            return server_1.NextResponse.json({
                error: 'Failed to clear existing workflow nodes'
            }, { status: 500 });
        }
        debug_logger_1.logger.debug('[Workflow Save] Existing nodes deleted successfully');
        // Insert new nodes
        ...
        debug_logger_1.logger.debug('[Workflow Save] Inserting nodes', { count: nodeInserts.length });
        debug_logger_1.logger.debug('[Workflow Save] First node sample', { node: nodeInserts[0] });
        const { error: nodesError } = await admin
            .from('workflow_nodes')
            .insert(nodeInserts);
        if (nodesError) {
            debug_logger_1.logger.error('[Workflow Save] Error inserting nodes', {}, nodesError);
            debug_logger_1.logger.error('[Workflow Save] Node insert data', { nodeInserts });
            return server_1.NextResponse.json({
                error: 'Failed to save workflow nodes'
            }, { status: 500 });
        }
        debug_logger_1.logger.debug('[Workflow Save] Nodes inserted successfully');
        ...
        if (edges && Array.isArray(edges) && edges.length > 0) {
            ...
            debug_logger_1.logger.debug('[Workflow Save] Inserting connections', { count: connectionInserts.length });
            debug_logger_1.logger.debug('[Workflow Save] First connection sample', { connection: connectionInserts[0] });
            const { error: connectionsError } = await admin
                .from('workflow_connections')
                .insert(connectionInserts);
            if (connectionsError) {
                debug_logger_1.logger.error('[Workflow Save] Error inserting connections', {}, connectionsError);
                debug_logger_1.logger.error('[Workflow Save] Connection insert data', { connectionInserts });
                return server_1.NextResponse.json({
                    error: 'Failed to save workflow connections'
                }, { status: 500 });
            }
            debug_logger_1.logger.debug('[Workflow Save] Connections inserted successfully');
        }
        // Auto-deactivate workflow if it has no nodes (or only has nodes but no valid start/end)
        let isActive;
        if (nodes.length === 0) {
            debug_logger_1.logger.debug('[Workflow Save] No nodes - auto-deactivating workflow');
            const { error: deactivateError } = await admin
                .from('workflow_templates')
                .update({ is_active: false, updated_at: new Date().toISOString() })
                .eq('id', templateId);
            if (deactivateError) {
                debug_logger_1.logger.error('[Workflow Save] Error auto-deactivating workflow', {}, deactivateError);
            }
            else {
                isActive = false;
            }
        ...
        debug_logger_1.logger.error('Error in PUT /api/admin/workflows/templates/[id]/steps', {}, error);
        return server_1.NextResponse.json({
