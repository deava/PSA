...
async function POST(request, { params }) {
    ...
    try {
      ...
      `)
        ...
        const validation = (0, validation_schemas_1.validateRequestBody)(validation_schemas_1.createWorkflowNodeSchema, body);
        if (!validation.success) {
            return server_1.NextResponse.json({ error: validation.error }, { status: 400 });
        }
        // Create node
        const node = await (0, workflow_service_1.createWorkflowNode)(id, validation.data);
        return server_1.NextResponse.json({ success: true, node }, { status: 201 });
    }
    catch (error) {
        debug_logger_1.logger.error('Error in POST /api/admin/workflows/templates/[id]/nodes', {}, error);
