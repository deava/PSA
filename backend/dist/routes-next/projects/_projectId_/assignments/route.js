...
async function GET(request, { params }) {
    try {
      ...
      `)
        ...
        if (error) {
            debug_logger_1.logger.error('Error fetching project assignments', {}, error);
        ...
        debug_logger_1.logger.error('Error in GET /api/projects/[projectId]/assignments', {}, error);
    ...
    try {
      ...
      `)
        ...
        if (project.status === 'complete') {
            return server_1.NextResponse.json({ error: 'Cannot add members to a completed project' }, { status: 400 });
        }
        // Check permissions
        ...
        if (existingAssignment) {
            if (existingAssignment.removed_at === null) {
                return server_1.NextResponse.json({ error: 'User is already assigned to this project' }, { status: 400 });
            }
            ...
            if (updateError) {
                debug_logger_1.logger.error('Error reactivating assignment', {}, updateError);
                return server_1.NextResponse.json({ error: 'Failed to add team member' }, { status: 500 });
            }
        }
        else {
            ...
            if (insertError) {
                debug_logger_1.logger.error('Error creating assignment', {}, insertError);
                return server_1.NextResponse.json({ error: 'Failed to add team member' }, { status: 500 });
            }
        }
        return server_1.NextResponse.json({ success: true, message: 'Team member added successfully' });
    }
    catch (error) {
        debug_logger_1.logger.error('Error in POST /api/projects/[projectId]/assignments', {}, error);
        return server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }
}
/**
...
async function DELETE(request, { params }) {
    try {
      ...
      `)
        ...
        if (project.status === 'complete') {
            return server_1.NextResponse.json({ error: 'Cannot remove members from a completed project' }, { status: 400 });
        }
        // Check permissions
        const userIsSuperadmin = (0, rbac_1.isSuperadmin)(userProfile);
        const hasEditAllProjects = await (0, rbac_1.hasPermission)(userProfile, permissions_1.Permission.MANAGE_ALL_PROJECTS, undefined, admin);
        ...
        const { error: updateError } = await admin
            ...
            .is('removed_at', null);
        if (updateError) {
            debug_logger_1.logger.error('Error removing assignment', {}, updateError);
            return server_1.NextResponse.json({ error: 'Failed to remove team member' }, { status: 500 });
        }
        ...
        if (workflowInstances && workflowInstances.length > 0) {
            ...
            if (nodeAssignmentError) {
                debug_logger_1.logger.error('Error removing workflow node assignments', {}, nodeAssignmentError);
                // Don't fail the whole operation, just log the error
            }
        }
        return server_1.NextResponse.json({ success: true, message: 'Team member removed successfully' });
    }
    catch (error) {
        debug_logger_1.logger.error('Error in DELETE /api/projects/[projectId]/assignments', {}, error);
        return server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }
}
