...
async function GET(_request) {
    ...
    if (!cronSecret) {
        debug_logger_1.logger.error('CRON_SECRET not configured - cron endpoint disabled for security');
        return server_1.NextResponse.json({ error: 'CRON_SECRET environment variable is required but not configured' }, { status: 500 });
    }
    const authHeader = _request.headers.get('authorization');
    if (authHeader !== `Bearer ${cronSecret}`) {
        return server_1.NextResponse.json({ error: 'Unauthorized: invalid or missing CRON_SECRET' }, { status: 401 });
    }
    const serviceRoleKey = process.env.DEMO_SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceRoleKey) {
        return server_1.NextResponse.json({ error: 'Service role key not configured' }, { status: 500 });
    }
    if (!DEMO_PROJECT_URL) {
        return server_1.NextResponse.json({ error: 'Supabase URL not configured (set DEMO_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_URL)' }, { status: 500 });
    }
    const supabase = (0, supabase_js_1.createClient)(DEMO_PROJECT_URL, serviceRoleKey);
    try {
    ...
    `;
        ...
        if (deptError)
            debug_logger_1.logger.error('Departments upsert error', {}, deptError);
        ...
        if (operationsDeptId) {
            ...
            if (opsRoleError)
                debug_logger_1.logger.error('Operations role upsert error', {}, opsRoleError);
        }
        ...
        for (const query of clearQueries) {
            ...
            if (error)
                debug_logger_1.logger.error('Clear error', {}, error);
        }
        ...
        if (projectsError) {
            debug_logger_1.logger.error('Projects error', {}, projectsError);
        }
        // Step 3: Insert tasks
        const tasks = generateTasks();
        const { error: tasksError } = await admin.from('tasks').upsert(tasks, { onConflict: 'id' });
        if (tasksError)
            debug_logger_1.logger.error('Tasks error', {}, tasksError);
        // Step 4: Insert project assignments
        const assignments = generateAssignments();
        const { error: assignmentsError } = await admin.from('project_assignments').upsert(assignments, { onConflict: 'id' });
        if (assignmentsError)
            debug_logger_1.logger.error('Assignments error', {}, assignmentsError);
        // Step 5: Insert account members
        const members = generateAccountMembers();
        const { error: membersError } = await admin.from('account_members').upsert(members, { onConflict: 'id' });
        if (membersError)
            debug_logger_1.logger.error('Members error', {}, membersError);
        // Step 6: Insert time entries
        const timeEntries = generateTimeEntries();
        const { error: timeError } = await admin.from('time_entries').upsert(timeEntries, { onConflict: 'id' });
        if (timeError)
            debug_logger_1.logger.error('Time entries error', {}, timeError);
        ...
        if (updatesError)
            debug_logger_1.logger.error('Updates error', {}, updatesError);
        // Step 8: Insert milestones
        const milestones = generateMilestones();
        const { error: milestonesError } = await admin.from('milestones').upsert(milestones, { onConflict: 'id' });
        if (milestonesError)
            debug_logger_1.logger.error('Milestones error', {}, milestonesError);
        // Step 9: Insert user availability
        const availability = generateUserAvailability();
        const { error: availabilityError } = await admin.from('user_availability').upsert(availability, { onConflict: 'id' });
        if (availabilityError)
            debug_logger_1.logger.error('Availability error', {}, availabilityError);
        // Step 10: Insert workflow templates
        const workflowTemplates = generateWorkflowTemplates();
        const { error: templatesError } = await admin.from('workflow_templates').upsert(workflowTemplates, { onConflict: 'id' });
        if (templatesError)
            debug_logger_1.logger.error('Workflow templates error', {}, templatesError);
        // Step 11: Insert workflow nodes
        const workflowNodes = generateWorkflowNodes();
        const { error: nodesError } = await admin.from('workflow_nodes').upsert(workflowNodes, { onConflict: 'id' });
        if (nodesError)
            debug_logger_1.logger.error('Workflow nodes error', {}, nodesError);
        // Step 12: Insert workflow connections
        const workflowConnections = generateWorkflowConnections();
        const { error: connectionsError } = await admin.from('workflow_connections').upsert(workflowConnections, { onConflict: 'id' });
        if (connectionsError)
            debug_logger_1.logger.error('Workflow connections error', {}, connectionsError);
        // Step 13: Insert workflow instances
        const workflowInstances = generateWorkflowInstances();
        const { error: instancesError } = await admin.from('workflow_instances').upsert(workflowInstances, { onConflict: 'id' });
        if (instancesError)
            debug_logger_1.logger.error('Workflow instances error', {}, instancesError);
        // Step 14: Insert newsletters
        const newsletters = generateNewsletters();
        const { error: newslettersError } = await admin.from('newsletters').upsert(newsletters, { onConflict: 'id' });
        if (newslettersError)
            debug_logger_1.logger.error('Newsletters error', {}, newslettersError);
        // Step 15: Insert project issues
        const projectIssues = generateProjectIssues();
        const { error: issuesError } = await admin.from('project_issues').upsert(projectIssues, { onConflict: 'id' });
        if (issuesError)
            debug_logger_1.logger.error('Project issues error', {}, issuesError);
        // Step 16: Insert task week allocations
        const taskAllocations = generateTaskWeekAllocations();
        const { error: allocationsError } = await admin.from('task_week_allocations').upsert(taskAllocations, { onConflict: 'id' });
        if (allocationsError)
            debug_logger_1.logger.error('Task allocations error', {}, allocationsError);
        // Step 17: Insert project stakeholders
        const stakeholders = generateProjectStakeholders();
        const { error: stakeholdersError } = await admin.from('project_stakeholders').upsert(stakeholders, { onConflict: 'id' });
        if (stakeholdersError)
            debug_logger_1.logger.error('Stakeholders error', {}, stakeholdersError);
    ...
    `;
        ...
        if (permError)
            debug_logger_1.logger.error('Role permissions update error', {}, permError);
    ...
    `;
        const { error: leadershipPermError } = await admin.rpc('exec_sql', { query: leadershipPermissionsUpdate });
        if (leadershipPermError)
            debug_logger_1.logger.error('Leadership permissions update error', {}, leadershipPermError);
      ...
      WHERE name = 'Admin';
    `;
        const { error: adminPermError } = await admin.rpc('exec_sql', { query: adminPermissionsUpdate });
        if (adminPermError)
            debug_logger_1.logger.error('Admin permissions update error', {}, adminPermError);
        return server_1.NextResponse.json({
            success: true,
        ...
        debug_logger_1.logger.error('Error resetting demo data', {}, error);
        return server_1.NextResponse.json({
