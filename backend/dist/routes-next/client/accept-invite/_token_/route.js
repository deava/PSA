      ...
      `)
        ...
        return server_1.NextResponse.json({
            invitation: {
                ...
                accountName: invitation.accounts?.name || 'Unknown Account',
                inviterName: invitation.inviter?.name || invitation.inviter?.email || 'An administrator',
            },
        });
    }
    catch (error) {
        debug_logger_1.logger.error('Error fetching client invitation details', {}, error);
...
async function POST(request, { params }) {
    ...
    try {
        ...
        if (name.length > 100) {
            return server_1.NextResponse.json({ error: 'Name is too long (max 100 characters)' }, { status: 400 });
        }
        ...
        if (company_position && typeof company_position === 'string' && company_position.length > 100) {
            return server_1.NextResponse.json({ error: 'Company position is too long (max 100 characters)' }, { status: 400 });
        }
        const adminSupabase = (0, supabase_server_1.createAdminSupabaseClient)();
        if (!adminSupabase) {
            return server_1.NextResponse.json({ error: 'Server configuration error' }, { status: 500 });
      ...
      `)
        ...
        if (authError) {
            debug_logger_1.logger.error('Failed to create client auth user', { error: authError.message });
            if (authError.message?.includes('already been registered') || authError.message?.includes('already exists')) {
                return server_1.NextResponse.json({ error: 'An account with this email already exists. Please contact your account manager.' }, { status: 409 });
            }
        ...
        if (profileError) {
            debug_logger_1.logger.error('Failed to create client user profile', { error: profileError.message, userId });
        ...
        if (clientRole) {
            ...
            if (roleError) {
                debug_logger_1.logger.error('Failed to assign Client role', { error: roleError.message, userId, roleId: clientRole.id });
                // Non-fatal: user is already marked as client
            }
        }
        else {
            debug_logger_1.logger.error('Client system role not found in roles table', { userId });
        }
        ...
        const { error: invitationError } = await adminSupabase
            ...
            .eq('id', invitation.id);
        if (invitationError) {
            debug_logger_1.logger.error('Failed to update client invitation status', { error: invitationError.message, invitationId: invitation.id });
        }
        // 5. Send welcome email
        const proto = request.headers.get('x-forwarded-proto') || 'http';
        const host = request.headers.get('host') || 'localhost:3000';
        const appUrl = `${proto}://${host}`;
        const accountName = invitation.accounts?.name || 'your account';
        ...
        if (!emailResult.success) {
            debug_logger_1.logger.error('Failed to send client welcome email', { error: emailResult.error });
        }
        debug_logger_1.logger.info('Client invitation accepted, user created', {
        ...
        debug_logger_1.logger.error('Error accepting client invitation', {}, error);
        return server_1.NextResponse.json({ error: 'Failed to create account' }, { status: 500 });
    }
}
