...
export async function isAssignedToWorkflowNode(
  ...
  const currentNode = (Array.isArray(instance.workflow_nodes)
    ? instance.workflow_nodes[0]
    : instance.workflow_nodes) as {
    id: string;
    node_type: string;
...
async function hasBasePermission(userProfile: UserWithRoles | null, permission: Permission, supabaseClient?: AppSupabaseClient | null): Promise<boolean> {
  ...
  for (const userRole of userProfile.user_roles) {
    ...
    if (role.permissions) {
      const permissions = role.permissions as unknown as Record<string, boolean> || {};
