...
export async function POST(request: NextRequest) {
  try {
    ...
    const canManageProjects = await hasPermission(userProfile, Permission.MANAGE_PROJECTS, { accountId }, admin)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userIsSuperadmin) {
      const canViewProjects = await hasPermission(userProfile, Permission.VIEW_PROJECTS, undefined, admin)
      if (!canViewProjects) {
        return NextResponse.json({ error: 'Insufficient permissions to view projects' }, { status: 403 })
      }
    }

    const hasViewAllProjects = userIsSuperadmin || await hasPermission(userProfile, Permission.VIEW_ALL_PROJECTS, undefined, admin)
