  ...
  try {
    ...
    try {
      ...
      if (userIsSuperadmin) {
        ...
        const hasViewAllAccounts = await hasPermission(user, Permission.VIEW_ALL_ACCOUNTS, undefined, admin);
        const hasViewAccounts = await hasPermission(user, Permission.VIEW_ACCOUNTS, { accountId }, admin);
      ...
      if (!hasAccess) {
        const hasServiceAccess = await accountService.canUserAccessAccount(user.id, accountId, admin);
