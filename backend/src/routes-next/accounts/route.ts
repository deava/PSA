...
export async function GET(request: NextRequest) {
  try {
    ...
    const canViewAccounts = await hasPermission(userProfile, Permission.VIEW_ACCOUNTS, undefined, admin)
    if (!canViewAccounts) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const canManageAccounts = await hasPermission(userProfile, Permission.MANAGE_ACCOUNTS, undefined, admin)
    if (!canManageAccounts) {
      logger.warn('Insufficient permissions to create account', { action: 'create_account', userId: user.id })
    ...
    if (!validation.success) {
      logger.warn('Invalid account creation data', { action: 'create_account', userId: user.id, error: validation.error })
    ...
    if (error) {
      ...
      logger.error('Failed to create account in database', { action: 'create_account', userId: user.id }, error as Error)
    ...
    logger.info('Account created successfully', { action: 'create_account', userId: user.id, accountId: account.id })
