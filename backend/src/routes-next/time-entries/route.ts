  ...
  try {
    ...
    if (taskId) {
      ...
      if (targetUserId !== userProfile.id) {
        const canViewTeam = await hasPermission(userProfile, Permission.VIEW_ALL_TIME_ENTRIES, undefined, admin);
        if (!canViewTeam) {
          return NextResponse.json(
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (existingEntry.user_id !== userProfile.id) {
      const canManageAllTime = await hasPermission(userProfile, Permission.VIEW_ALL_TIME_ENTRIES, undefined, admin);
      if (!canManageAllTime) {
        return NextResponse.json(
