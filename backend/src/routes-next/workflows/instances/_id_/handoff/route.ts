...
export async function POST(
  ...
  try {
    ...
    const canExecute = await hasPermission(userProfile, Permission.EXECUTE_WORKFLOWS, { workflowInstanceId: id }, admin);
    if (!canExecute) {
    ...
    if (validation.data.out_of_order) {
      const canSkip = await hasPermission(userProfile, Permission.SKIP_WORKFLOW_NODES, undefined, admin);
      if (!canSkip) {
        return NextResponse.json({
