...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isOwnData) {
      const canViewTeam = await hasPermission(userProfile, Permission.VIEW_TEAM_CAPACITY, undefined, admin);
      const canViewAll = await hasPermission(userProfile, Permission.VIEW_ALL_CAPACITY, undefined, admin);
    ...
    const availability = await availabilityService.getUserAvailability(userId, weekStartDate, admin);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const canEdit = await hasPermission(userProfile, Permission.EDIT_OWN_AVAILABILITY, undefined, admin);
    if (!canEdit) {
      return NextResponse.json(
