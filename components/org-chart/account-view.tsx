...
function AccountCard({
  ...
  const handleAssignUser = async (userId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/accounts/${account.id}/members`, {
        method: 'POST',
  ...
  const confirmRemoveUser = async () => {
    ...
    try {
      const response = await apiFetch(`/api/accounts/${account.id}/members/${userToRemove}`, {
        method: 'DELETE',
      });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active':
        return 'bg-green-100 text-emerald-400 border-green-200';
      case 'inactive':
                    ...
                    >
                      ...
                      {!isReadOnly && canRemoveUsers && (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-destructive/10 shrink-0"
...
export function AccountView({
  ...
  const loadAccounts = async () => {
    try {
      ...
      const response = await apiFetch('/api/accounts/members');
