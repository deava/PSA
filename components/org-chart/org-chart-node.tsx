...
function OrgChartNodeComponent({ data }: NodeProps) {
  ...
  if (role) {
    ...
    const getRoleColor = () => {
      if (role.is_system_role) return 'border-red-200 bg-destructive/10';
      if (role.hierarchy_level >= 100) return 'border-blue-200 bg-primary/10';
      if (role.hierarchy_level >= 80) return 'border-green-200 bg-emerald-500/10';
