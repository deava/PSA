...
export function UserAssignmentDialog({ 
  ...
  const loadAssignedUsers = useCallback(async () => {
    ...
    try {
      const response = await apiFetch(`/api/roles/${role.id}/users`);
  ...
  async function assignUser(userId: string) {
    ...
    try {
      setAssigning(true);
      const response = await apiFetch(`/api/roles/${role.id}/assign-user`, {
        method: 'POST',
  ...
  async function removeUser(userId: string) {
    ...
    try {
      setAssigning(true);
      const response = await apiFetch(`/api/roles/${role.id}/remove-user/${userId}`, {
        method: 'DELETE',
      });
                  ...
                  <ScrollArea className="h-48 border rounded-lg">
                    <div className="p-2 space-y-1">
                      {assignedUsers.map((user: any) => (
                        <div
                          key={(user as any).id}
                          className="flex items-center justify-between p-2 rounded hover:bg-card"
                        >
                          ...
                          <Button
                            ...
                            disabled={assigning}
                            className="h-7 text-red-600 hover:text-red-700 hover:bg-destructive/10"
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
