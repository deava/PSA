...
interface ClockWidgetProps {
  ...
  isVisible?: boolean;
  hidden?: boolean;
  managed?: boolean; // when true, skip internal fixed positioning (parent handles it)
}

export function ClockWidget({ onDock, isVisible = true, hidden = false, managed = false }: ClockWidgetProps) {
  ...
  const handleClockIn = async () => {
    setClockingIn(true)
    try {
      const response = await apiFetch('/api/clock', {
        method: 'POST',
  ...
  return (
    <>
      ...
      <div
        ref={widgetRef}
        className={`${managed ? '' : 'fixed z-50'} transition-all duration-300 ${isDragging ? 'cursor-grabbing' : ''}`}
        style={managed ? {} : {
      ...
      >
        <Card className={`shadow-lg border-2 transition-all duration-200 ${
          clockedIn ? 'border-primary/50 bg-primary/5' : 'border-white/10 bg-card'
        }`}>
          {isMinimized ? (
            ...
            <div className="flex items-center">
              <div
                ...
                className="p-2 cursor-grab hover:bg-muted rounded-l-lg"
                title="Drag to move"
              >
              ...
              >
                <Clock className={`w-5 h-5 ${clockedIn ? 'text-green-600' : 'text-muted-foreground'}`} />
                {clockedIn && (
            ...
            <div className="p-4 min-w-[200px]">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    onMouseDown={handleDragStart}
                    className="cursor-grab hover:bg-muted p-1 rounded -ml-1"
                    title="Drag to move"
                  >
                    <GripVertical className="w-4 h-4 text-muted-foreground" />
                  </div>
                  <Timer className={`w-4 h-4 ${clockedIn ? 'text-green-600' : 'text-muted-foreground'}`} />
