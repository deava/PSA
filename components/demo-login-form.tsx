  ...
  const handleDemoLogin = async (user: DemoUser) => {
    ...
    try {
      ...
      if (rawMessage.includes('Supabase not configured')) {
        ...
        errorMessage = 'Demo user not found in database.';
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl">Welcome to Worklo Demo</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        {message && (
          <div className="text-sm text-primary bg-primary/10 border border-primary/20 p-3 rounded-lg text-center">
            {message}
          </div>
        )}

        {/* Demo mode notice */}
        <div className="text-center text-xs text-muted-foreground pt-4 border-t border-white/8">
          <p>This is a demo environment. Changes may be reset periodically.</p>
          <p className="mt-1">
            All demo accounts use password:{' '}
            <code className="bg-card/8 px-1 rounded">Test1234!</code>
          </p>
        </div>
      </CardContent>
    </Card>
