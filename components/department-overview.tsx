...
export function DepartmentOverview({
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'urgent': return 'bg-red-100 text-destructive border-red-200 hover:bg-red-200 hover:text-destructive';
      case 'high': return 'bg-orange-100 text-orange-400 border-orange-200 hover:bg-orange-200 hover:text-orange-400';
      case 'medium': return 'bg-yellow-100 text-amber-400 border-yellow-200 hover:bg-yellow-200 hover:text-amber-400';
      case 'low': return 'bg-green-100 text-emerald-400 border-green-200 hover:bg-green-200 hover:text-emerald-400';
      default: return 'bg-muted text-foreground border-white/10 hover:bg-muted hover:text-foreground';
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
          <div className="space-y-3">
            ...
            {department.description && (
              <p className="text-muted-foreground text-lg leading-relaxed">{department.description}</p>
            )}
          </div>
          {canManageDepartments && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Projects assigned to this department</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{metrics.teamSize}</p>
                <p className="text-xs text-muted-foreground">Users assigned to this department</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{metrics.capacityUtilization}%</p>
                <p className="text-xs text-muted-foreground">Current team workload</p>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-primary/10 rounded-lg">
                  ...
                  <div className="text-sm text-primary">Planning</div>
                </div>
                ...
                <div 
                  ...
                  className={`p-4 border rounded-lg ${
                    issue.status === 'in_progress' 
                      ? 'bg-amber-500/10 border-yellow-200' 
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="font-medium text-foreground mb-1">Issue #{issue.id.slice(-6)}</h4>
                        <p className="text-sm text-muted-foreground line-clamp-2 break-words">{issue.content}</p>
                      </div>
                      <div className="flex-shrink-0">
                        <Button
                          variant="outline"
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground">Workflow Step</th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground">Priority</th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground">Account</th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground">Est Hours</th>
              ...
              <tbody>
                {filteredAndSortedProjects.map((project:any) => (
                  <tr key={project.id} className="border-b hover:bg-card">
                    ...
                    <td className="py-3 px-4">
                      {project.workflow_step ? (
                        <Badge className="text-xs whitespace-nowrap border bg-blue-100 text-primary border-blue-300">
                          {project.workflow_step}
                        </Badge>
                      ) : (
                    ...
                    <td className="py-3 px-4">
                      <span className="text-sm text-muted-foreground">{project.accountName}</span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1">
          ...
          <CardContent>
            <div className="space-y-3">
              {finishedProjects.map((project:any) => (
                <div
                  key={project.id}
                  className="flex items-center justify-between p-4 border rounded-lg hover:bg-card"
                >
                  <div className="flex-1 min-w-0">
                    <Link
