...
export function WorkflowTimeline({ workflowInstanceId }: WorkflowTimelineProps) {
  ...
  const getNodeTypeColor = (nodeType: string) => {
    switch (nodeType) {
      case 'start': return 'bg-green-100 text-emerald-400 border-green-300';
      case 'approval': return 'bg-blue-100 text-primary border-blue-300';
      case 'role': return 'bg-purple-100 text-purple-400 border-purple-300';
      case 'department': return 'bg-orange-100 text-orange-400 border-orange-300';
      case 'form': return 'bg-yellow-100 text-amber-400 border-yellow-300';
      case 'end': return 'bg-muted text-foreground border-white/15';
      default: return 'bg-muted text-foreground border-white/15';
    }
  };
                ...
                <div key={node.id} className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div className="flex flex-col items-center gap-2 min-w-[120px]">
                    ...
                    <div className={`
                      ...
                      ${isFuture ? 'bg-muted border-white/15' : ''}
                    `}>
                      {isPast ? (
                    ...
                    <div className="text-center">
                      <p className={`text-sm font-medium ${isCurrent ? 'text-blue-600' : isPast ? 'text-green-600' : 'text-muted-foreground'}`}>
                        {node.label}
                      </p>
                      <Badge
                        variant="outline"
