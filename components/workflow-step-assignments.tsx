...
export function WorkflowStepAssignments({
  ...
  const handleAssign = async () => {
    ...
    try {
      const response = await apiFetch('/api/workflows/steps/assignments', {
        method: 'POST',
        ...
        {displayNodes.map(node => (
          <div
            ...
            className={cn(
              ...
              node.assignments.length > 0 ? "bg-emerald-500/10 border-green-200" : "bg-card border-white/10"
            )}
          >
            ...
            {node.assignments.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {node.assignments.map(a => (
                  <Badge
                    key={a.id}
                    className="bg-green-100 text-emerald-400 border-green-200 flex items-center gap-1"
                  >
                    {a.user_profiles.name}
                    {canManageAssignments && (
                      <button
