...
export function WorkflowProgressButton({
    ...
    try {
      ...
      if (formTemplate) {
        ...
        if (isInlineForm) {
          ...
          const formResponse = await apiFetch('/api/workflows/forms/responses', {
            method: 'POST',
      ...
      const response = await apiFetch('/api/workflows/progress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          workflowInstanceId,
  ...
  if (!hasRequiredRole) {
    if (isPipelineProject && assignedFutureStepName) {
      ...
      return (
        <div className="flex items-center gap-2 p-3 bg-primary/10 border border-blue-200 rounded-lg text-sm">
          ...
          {!loading && (
            <div className="space-y-4 py-4 overflow-y-auto flex-1 min-h-0">
              ...
              {currentNode && (nextNode || nextNodes.length > 0) && (
                <div className="p-4 bg-primary/10 border border-blue-200 rounded-lg space-y-3">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 ml-6">
                      <Badge className="bg-blue-100 text-primary text-xs">
                        {currentNode.node_type}
                      </Badge>
                      <span className="text-sm text-primary">{currentNode.label}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {nextNodes.length > 1 ? (
                      <div className="ml-6 space-y-2">
                        {nextNodes.map((node:any, index:number) => (
                          <div key={node.id} className="flex items-center gap-2">
                            <Badge className="bg-purple-100 text-purple-400 text-xs">
                              {node.node_type}
                            </Badge>
                            <span className="text-sm text-primary">{node.label}</span>
                            {index < nextNodes.length - 1 && <span className="text-xs text-muted-foreground">&</span>}
                          </div>
                        ))}
                      ...
                      <div className="flex items-center gap-2 ml-6">
                        ...
                        <span className="text-sm text-primary">{nextNode.label}</span>
                      </div>
                    ) : null}
                  </div>
              ...
              {nextNodes.length > 1 && Object.keys(usersPerNode).length > 0 && (
                <div className="space-y-4">
                  ...
                  {nextNodes.map((node:any) => {
                    ...
                    return (
                      <div key={node.id} className="space-y-2 p-3 bg-card rounded-lg border">
                        <div className="flex items-center gap-2">
                          <Badge className="bg-purple-100 text-purple-400 text-xs">
                            {node.node_type}
                          </Badge>
              ...
              {nextNodes.length <= 1 && availableUsers.length > 0 && (
                <div className={`space-y-2 ${isSyncNode ? 'p-4 bg-amber-500/10 border border-amber-200 rounded-lg' : ''}`}>
                  ...
                  <p className={`text-xs ${isSyncNode ? 'text-amber-700' : 'text-muted-foreground'}`}>
                    {isSyncNode
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-emerald-400">Form Already Submitted</h4>
                      <p className="text-xs text-green-700">This form was completed in a previous step</p>
                    </div>
                    <Badge className="bg-green-100 text-emerald-400">Completed</Badge>
                  </div>
                    ...
                    {existingFormData.fields?.map((field: any) => {
                      ...
                      return (
                        <div key={field.id as string} className="flex flex-col">
                          <span className="text-xs text-muted-foreground font-medium">{field.label as string}</span>
              ...
              {(formTemplate && !existingFormData && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b">
                    ...
                    <div>
                      <h4 className="font-medium text-purple-400">{formTemplate.name}</h4>
                  ...
                  {formTemplate.fields.map((field:any) => {
                    ...
                    return (
                      <div key={field.id} className="space-y-2">
                        ...
                        {field.type === 'file' && (
                          <div className="p-4 bg-card border border-dashed border-white/15 rounded-lg text-center">
              ...
              {(isFormNode && !formTemplate && !loading && (
                <div className="p-4 bg-amber-500/10 border border-yellow-200 rounded-lg">
