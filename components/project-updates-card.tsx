...
export default function ProjectUpdatesCard({ className }: ProjectUpdatesCardProps) {
  ...
  const loadUpdates = async () => {
    try {
      ...
      const response = await apiFetch('/api/project-updates')
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'urgent': return 'bg-red-100 text-destructive border-red-200'
      case 'high': return 'bg-orange-100 text-orange-400 border-orange-200'
      case 'medium': return 'bg-yellow-100 text-amber-400 border-yellow-200'
      case 'low': return 'bg-green-100 text-emerald-400 border-green-200'
      default: return 'bg-muted text-foreground border-white/10'
    }
  }
          ...
          {updates.length === 0 ? (
            ...
            <div className="space-y-4">
              {updates.map((update:any) => (
                <div key={update.id} className="border rounded-lg p-4 hover:bg-card transition-colors">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-foreground mb-3 break-words">{renderMarkdownContent(update.content)}</p>
