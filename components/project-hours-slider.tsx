...
export default function ProjectHoursSlider({
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xs font-medium ${progressPercent >= 100 ? 'text-green-600' : progressPercent >= 75 ? 'text-blue-600' : 'text-muted-foreground'}`}>
          {progressPercent}%
        </span>
      </div>
    );
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`text-xs p-2 rounded ${progressPercent >= 100 ? 'bg-emerald-500/10 text-emerald-400' : progressPercent >= 75 ? 'bg-primary/10 text-primary' : 'bg-card text-foreground'}`}>
