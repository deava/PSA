    ...
    >
        ...
        <div className="flex items-center gap-2 relative" style={{ zIndex: 1 }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            {nodeData.config?.departmentName && (
              <div className="text-xs text-muted-foreground truncate">{nodeData.config.departmentName}</div>
            )}
            {nodeData.config?.roleName && (
              <div className="text-xs text-muted-foreground truncate">{nodeData.config.roleName}</div>
            )}
            ...
            {nodeData.config?.formTemplateName && (
              <div className="text-xs text-muted-foreground truncate">Form: {nodeData.config.formTemplateName}</div>
            )}
      ...
      {nodeData.type === 'conditional' && (!nodeData.config?.conditions || nodeData.config.conditions.length === 0) && (
        <div className="absolute bottom-[-20px] left-1/2 transform -translate-x-1/2 pointer-events-none">
          <span className="text-[9px] text-muted-foreground whitespace-nowrap">Configure branches</span>
        </div>
      )}
    </div>
  );
