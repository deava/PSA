...
export function ClockOutDialog({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const response = await apiFetch('/api/clock/out', {
        method: 'POST',
  ...
  const handleDiscardAndClockOut = async () => {
    setSubmitting(true)
    try {
      const response = await apiFetch('/api/clock/discard', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          ...
          {/* Summary */}
          <div className={`p-3 rounded-lg ${isBalanced ? 'bg-emerald-500/10 border border-green-200' : 'bg-amber-500/10 border border-yellow-200'}`}>
