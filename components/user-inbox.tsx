  ...
  useEffect(() => {
    ...
    const loadInboxData = async () => {
      try {
        ...
        const projectsRes = await apiFetch('/api/workflows/my-projects');
        if (cancelled) return;
        if (projectsRes.ok) {
        ...
        const approvalsRes = await apiFetch('/api/workflows/my-approvals');
        if (cancelled) return;
        if (approvalsRes.ok) {
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {workflowSteps[projectId] ? (
                                <Badge className="border bg-blue-100 text-primary border-blue-300">
                                  {workflowSteps[projectId]}
                                </Badge>
                              ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge className="bg-yellow-100 text-amber-400">
                                Awaiting Approval
                              </Badge>
                              {displayProject?.priority && (
                                <Badge className={getPriorityColor(displayProject.priority)}>
