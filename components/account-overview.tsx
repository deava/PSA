...
export function AccountOverview({ account, metrics, urgentItems, userProfile, hasFullAccess = true }: AccountOverviewProps) {
  ...
  const handleUpdateIssueStatus = async (issueId: string, projectId: string, newStatus: 'open' | 'in_progress' | 'resolved') => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/projects/${projectId}/issues/${issueId}`, {
        method: 'PUT',
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <Link
                          ...
                          className="text-sm font-medium text-foreground hover:text-[#007EE5] hover:underline"
                        >
                          {project.name}
                        </Link>
          ...
          {projectToDelete && (
            <div className="space-y-3 pt-2">
              <p className="text-sm text-muted-foreground">
                You are about to delete <span className="font-semibold text-foreground">&quot;{projectToDelete.name}&quot;</span>.
              </p>
