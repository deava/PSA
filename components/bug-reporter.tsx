...
export function BugReporter() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Bug report submitted â€” thank you!');
      resetForm();
      setOpen(false);
    } catch (error) {
      toast.error(
  ...
  return (
    <>
      {/* Hover trigger zone + bug button â€” slides in from left */}
      <div
        className="fixed z-50 group"
        style={{ bottom: '4rem', left: 0, width: '3rem', height: '3rem' }}
      >
        <button
          onClick={() => setOpen(true)}
          className="absolute flex items-center justify-center w-10 h-10 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-lg transition-all duration-300 ease-out opacity-0 group-hover:opacity-100 -translate-x-full group-hover:translate-x-1"
