...
export default function RBACDiagnosticsPage() {
  ...
  const fetchDiagnostics = async () => {
    setLoading(true);
    try {
      const response = await apiFetch('/api/admin/rbac-diagnostics');
  ...
  const runDiagnosticTest = async () => {
    ...
    try {
      const response = await apiFetch('/api/admin/rbac-diagnostics/test', {
        method: 'POST'
      });
            ...
            {testResults.failures.length > 0 && (
              <div className="mt-2 space-y-1">
                {testResults.failures.map((failure: string, i: number) => (
                  <div key={i} className="text-sm">â€¢ {failure}</div>
                ))}
              </div>
            )}
          </AlertDescription>
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            {role.department_name} â€¢ {role.user_count} user(s)
                          </CardDescription>
                        </div>
                        <Badge>
