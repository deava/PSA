...
export default function RoleManagementPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const rolesResponse = await apiFetch('/api/roles', {
      ...
      // Load departments
      const deptsResponse = await apiFetch('/api/departments', {
        headers: cacheHeaders,
        cache: 'no-store',
        credentials: 'include'
      });
  ...
  async function confirmDeleteRole() {
    ...
    try {
      const response = await apiFetch(`/api/roles/${roleToDelete}`, {
        method: 'DELETE',
      });
  ...
  async function handleSaveReportingRole(roleId: string, reportingRoleId: string | null) {
    ...
    if (reportingRoleId) {
      ...
      const response = await apiFetch('/api/roles/reorder', {
        method: 'PATCH',
