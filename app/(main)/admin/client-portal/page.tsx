...
export default function ClientPortalPage() {
  ...
  const loadInvitations = useCallback(async (accountId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/accounts/${accountId}/client-invites`);
      if (response.ok) {
        const data = await response.json();
  ...
  const loadFeedback = useCallback(async (accountId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/accounts/${accountId}/client-feedback`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSendInvitation = async () => {
    ...
    setSending(true);
    try {
      const response = await apiFetch(`/api/accounts/${selectedAccount.id}/invite-client`, {
        method: 'POST',
  ...
  const getStatusBadge = (invitation: ClientInvitation) => {
    ...
    if (invitation.status === 'accepted') {
      return <Badge className="bg-green-100 text-emerald-400 flex items-center gap-1"><Check className="w-3 h-3" />Accepted</Badge>;
    } else if (invitation.status === 'expired' || expiresAt < now) {
      return <Badge className="bg-muted text-foreground flex items-center gap-1"><X className="w-3 h-3" />Expired</Badge>;
    } else {
      return <Badge className="bg-yellow-100 text-amber-400 flex items-center gap-1"><Clock className="w-3 h-3" />Pending</Badge>;
    }
  };
  ...
  const getSatisfactionBadge = (score: number | null) => {
    ...
    if (score >= 8) {
      return (
        <Badge className="bg-green-100 text-emerald-400 gap-1">
      ...
      return (
        <Badge className="bg-yellow-100 text-amber-400 gap-1">
      ...
      return (
        <Badge className="bg-red-100 text-destructive gap-1">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="feedback">
                ...
                {stats && stats.totalFeedback > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-card rounded-lg mb-4">
                    <div>
                          ...
                          <TableRow key={item.id}>
                            ...
                            <TableCell>
                              <button
                                ...
                                className="text-sm text-blue-600 hover:text-primary hover:underline"
                              >
                                View Details
                              </button>
                            </TableCell>
            ...
            <div className="space-y-6 py-4">
              ...
              {selectedFeedback.what_went_well && (
                <div>
                  ...
                  <div className="mt-2 p-4 bg-emerald-500/10 rounded-md border border-green-200">
              ...
              {selectedFeedback.what_needs_improvement && (
                <div>
                  ...
                  <div className="mt-2 p-4 bg-orange-500/10 rounded-md border border-orange-200">
