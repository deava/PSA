...
export default function WorkflowsClient({ canManageWorkflows }: WorkflowsClientProps) {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const response = await apiFetch('/api/admin/workflows/templates?include_inactive=true');
  ...
  const handleCreateTemplate = async () => {
    ...
    setSaving(true);
    try {
      const url = apiUrl('/api/admin/workflows/templates');
      console.log('[createTemplate] POST to:', url);
      const response = await apiFetch('/api/admin/workflows/templates', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        const msg = data.details || data.error || 'Failed to create workflow template';
        toast.error(msg);
      }
    } catch (err: unknown) {
  ...
  const handleDeleteTemplate = async () => {
    ...
    setDeleting(true);
    try {
      const response = await apiFetch(`/api/admin/workflows/templates/${templateToDelete.id}`, {
        method: 'DELETE',
      });
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {templates.map((template) => (
                <div
                  key={template.id}
                  className="flex items-center justify-between p-4 border border-white/8 rounded-lg hover:bg-card/5 transition-colors"
                >
